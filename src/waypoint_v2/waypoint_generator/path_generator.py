...
from custom_a_star import astar_path_custom
from geodesy.utm import UTMPoint, gridZone, GeoPoint
from intersection import intersection_info

class path_generator():
    def __init__(self, type = 'A', crossroad = False, intersection = None):
    ...
    def generate_graph(self, type, crossroad, intersection):
        graph = nx.DiGraph()
        id_map = dict()
    ...
    def load_graph(self, graph, id_map, postfix, crossroad, intersection):
        ...
        if intersection:
            cross_lines = []
            f = open(intersection, 'rt')
            self.intersection_map = intersection_info[int(intersection.split('.')[0][-1])-1]
            zone, band = gridZone(35.64838, 128.40105)
            for l in f.read().splitlines():
                x, y, z = l.split(',')
                x, y, z = float(x) + self.center[0], float(y) + self.center[1], float(z)
                gp = UTMPoint(x,y,z,zone,band).toMsg()
    ...
    def generate_path(self, source_index, target_index):
        ...
        # route = astar_path_custom(self.G, source=source_index, target=target_index, heuristic=self.dist_heuristic, weight='length')
        route_list = []
        stops =[]
        changes = []
        dists = []
        ...
        for i in range(len(route)):
            ...
            if self.intersection_map is not None and stop == 'CROSS':
                print('Lane Number:',self.intersection_map[tuple((self.G.nodes[route[i]]['vertex'][0], self.G.nodes[route[i]]['vertex'][1]))])
