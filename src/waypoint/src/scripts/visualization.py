...
# PG = path_generator(type = 'A')
# PG = path_generator(type = 'B')
nodes = None
edges = None
target_node_ids = []
target_nodes = None
...
stop_pub = rospy.Publisher("/map/stop", MarkerArray, queue_size=10, latch=True)
gps_pub = rospy.Publisher('/map/gps', Marker, queue_size=10, latch=True)
gps_point_publisher = rospy.Publisher('/imcar/points_marker', Marker, queue_size=10, latch=True)
...
def callback(gps_sub, can_sub):
    ...
    if g_heading != 999:
        ...
        if destination_node_index is not None and target_nodes is not None:
            ...
            gps_marker.text = str(dist_stop) +','+ str(dist_target)
...
def callback_goalpose(message):
    ...
    #targets.header.stamp = rospy.Time.now()
    node_pub.publish(nodes)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--type', type=str, default='A', help='A or B')
    parser.add_argument('--crossroad', action='store_true')
    args = parser.parse_args()
    PG = path_generator(type = args.type, crossroad = args.crossroad)
