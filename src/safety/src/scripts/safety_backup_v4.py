...
class Safety:
    ...
    def callback(self, *args):
        ...
        if self.freespace_msg is not None and self.target_waypoint_msg.markers:
            ...
            if car_cw_dist < 10 and self.ped_all_cam:
            ...
            if self.ped_stop_start and rospy.Time.now().secs - self.ped_stop_time < rospy.Time(3).secs: ## greater? lower?
                target_speed = self.stop_speed
                print('stop for 3 secs')
            elif self.ped_stop_start and rospy.Time.now().secs - self.ped_stop_time >= rospy.Time(3).secs:
                if car_cw_dist < 10 and self.ped_diff < 0.05:
                    target_speed = self.orig_speed
                    self.ped_stop_start = True
                else:
                    target_speed = self.stop_speed
            ...
            if self.car_stop_start and rospy.Time.now().secs - self.car_stop_time > rospy.Time(10).secs and target_speed != self.orig_speed:
                target_speed = self.orig_speed
                lane_change = True
                self.car_stop_start = False
            ...
            elif self.car_stop_start and rospy.Time.now().secs - self.car_stop_time < rospy.Time(5).secs:
