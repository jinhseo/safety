...
class Safety:
    def __init__(self, pub_rate, freespace, target_waypoint, waypoint, car, points_ring, heading, heading_v2, can_data, d_object, frame_id):
        ...
        cross_walk = open('cross_node_v2.txt', 'r')
    ...
    def callback(self, *args):
        ...
        #if self.target_waypoint_msg is not None:
        #import IPython; IPython.embed()
        if self.target_waypoint_msg.markers:
            ...
            target_waypoint = self.to_narray(self.target_waypoint_msg.markers)
            ...
            trans_cw = self.cw_node[:, :2] - car_position
            transformed_cw = np.dot(trans_cw, np.array([x_axis, y_axis]))
            ...
            neighbour_cw = transformed_cw[(distance.cdist(car_position, self.cw_node[:, :2]) < 20)[0]]
            ped_cw_dist = distance.cdist(transformed_freespace, neighbour_cw).min(0).max()
            #ped_cw_dist = distance.cdist(transformed_freespace, transformed_cw).min()
            print(ped_cw_dist)
            if len(self.d_object_msg.detections) != 0:
                ...
                #neighbour_cw = transformed_cw[(distance.cdist(car_position, self.cw_node[:, :2]) < 20)[0]]
                #distance.cdist(transformed_freespace, neighbour_cw).min(0).max()
                #import IPython; IPython.embed()
                #print()
            #print(cw_min_dist)
            #import IPython; IPython.embed()
